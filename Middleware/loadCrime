...
export async function loadCrimeData(req, res, next) {
  try {
    const url = 'https://data.princegeorgescountymd.gov/resource/amvf-x3gi.json'; // remote URL! you can test it in your browser
    const data = await fetch(url); // We're using a library that mimics a browser 'fetch' for simplicity
    const json = await data.json(); // the data isn't json until we access it using dot notation

    const reply = json.filter((item) => Boolean(item.geocoded_column_1)).filter((item) => Boolean(item.name));

    console.log('Results in loadCrimeData middleware', json.length); // let's check that something's there before we return it
    req.loadCrimeData = reply; // and let's _attach_ the data to our request object here
    next();
  } catch (err) {
